...
DROP TABLE IF EXISTS feuilles_maladie CASCADE;
DROP TABLE IF EXISTS prescription_specialists CASCADE;
DROP TABLE IF EXISTS prescription_medicaments CASCADE;
...
CREATE TABLE doctors (
    ...
    specialty VARCHAR(50) NOT NULL, -- 'GENERALISTE' or 'SPECIALISTE'
...
CREATE TABLE patients (
    ...
    medecin_traitant_id INT REFERENCES doctors(id) ON DELETE SET NULL
);
...
-- Create prescription_medicaments table
...
-- Create prescription_specialists table
CREATE TABLE prescription_specialists (
    id SERIAL PRIMARY KEY,
    prescription_id INT REFERENCES prescriptions(id) ON DELETE CASCADE,
    specialty_needed VARCHAR(50) NOT NULL,
    reason VARCHAR(255) NOT NULL,
    referred_doctor_id INT REFERENCES doctors(id) ON DELETE SET NULL
);

-- Create feuilles_maladie table
CREATE TABLE feuilles_maladie (
    id SERIAL PRIMARY KEY,
    consultation_id INT REFERENCES consultations(id) ON DELETE CASCADE,
    date TIMESTAMP NOT NULL,
    status VARCHAR(50) DEFAULT 'EN_ATTENTE' -- 'EN_ATTENTE', 'VALIDEE', 'REFUSEE'
);

-- Create remboursements table
CREATE TABLE remboursements (
    id SERIAL PRIMARY KEY,
    feuille_maladie_id INT REFERENCES feuilles_maladie(id) ON DELETE CASCADE,
    amount DOUBLE PRECISION NOT NULL,
    rate INT NOT NULL, -- 100 or 80
    method VARCHAR(50) NOT NULL, -- 'VIREMENT' or 'CASH'
    bank_account VARCHAR(100), -- RIB (optional)
    status VARCHAR(50) DEFAULT 'EN_ATTENTE' -- 'EN_ATTENTE', 'EFFECTUE'
);
